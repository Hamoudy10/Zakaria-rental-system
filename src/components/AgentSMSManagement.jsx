...
import { Send, AlertCircle, RefreshCw, Clock, CheckCircle, XCircle } from 'lucide-react';
...
const AgentSMSManagement = () => {
  ...
  const [missingBillsConfirmation, setMissingBillsConfirmation] = useState(null);
  ...
  const fetchSMSHistory = async () => {
    ...
    try {
      ...
      if (historyFilters.startDate) params.start_date = historyFilters.startDate;
      if (historyFilters.endDate) params.end_date = historyFilters.endDate;
      if (historyFilters.propertyId) params.property_id = historyFilters.propertyId;
      
      const response = await API.billing.getSMSHistory(params);
      if (response.data.success) {
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage billing SMS, retry failed messages, and view history</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          {activeTab === 'trigger' && (
            <div className="space-y-6">
                ...
                <div className="flex">
                  ...
                  <div>
                    ...
                    <ul className="mt-2 text-sm text-blue-700 space-y-1">
                      <li>• System will check for water bills before sending SMS</li>
                      <li>• Tenants without water bills will show in confirmation</li>
                      <li>• You can proceed with water amount set to 0 for missing bills</li>
              ...
              {/* Result Display */}
              {triggerResult && (
                <div className={`rounded-md p-4 ${triggerResult.type === 'success' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                  <div className="flex">
                    ...
                    <div>
                      <h4 className={`text-sm font-medium ${triggerResult.type === 'success' ? 'text-green-800' : 'text-red-800'}`}>
                        {triggerResult.type === 'success' ? 'Success!' : 'Error'}
                      </h4>
                      <p className={`mt-1 text-sm ${triggerResult.type === 'success' ? 'text-green-700' : 'text-red-700'}`}>
                        {triggerResult.message}
                      </p>
                      {triggerResult.data && (
                        <div className="mt-3 text-sm">
                          <p><strong>Queued:</strong> {triggerResult.data.queued} SMS</p>
                          <p><strong>Properties:</strong> {triggerResult.data.property_count}</p>
                          <p><strong>Month:</strong> {triggerResult.data.month}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                    <div className="p-6">
                      <div className="flex items-center mb-4">
                        ...
                        <h3 className="text-lg font-medium text-gray-900">Missing Water Bills</h3>
                      </div>
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-6">
                        ...
                        <ul className="text-sm text-yellow-700 space-y-1 max-h-60 overflow-y-auto">
                          {missingBillsConfirmation.data.missing_tenants.map((tenant, index) => (
                            <li key={index} className="flex items-center justify-between py-1">
                              <span>{tenant.name}</span>
                              <span className="text-yellow-600">{tenant.unit}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
          ...
          {activeTab === 'failed' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium text-gray-900">Failed SMS Messages</h3>
                    ...
                    <thead>
                      <tr className="bg-gray-50">
                        <th className="px-4 py-3 text-left">
                          <input
                            type="checkbox"
                            checked={selectedFailedSMS.length === failedSMS.length && failedSMS.length > 0}
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {failedSMS.map((sms) => (
                        <tr key={sms.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-4">
                            <div className="text-sm text-gray-900">{sms.property_name}</div>
                            <div className="text-sm text-gray-500">{sms.unit_code}</div>
                          </td>
                          <td className="px-4 py-4 text-sm text-gray-500">
                            {sms.recipient_phone}
                          </td>
                          <td className="px-4 py-4">
                            <div className="max-w-xs truncate" title={sms.error_message}>
                              <span className="text-sm text-red-600">{sms.error_message}</span>
                            </div>
                          </td>
                          <td className="px-4 py-4">
                            <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setHistoryFilters({...historyFilters, propertyId: e.target.value})}
                    className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
                  >
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {smsHistory.map((sms) => (
                        <tr key={sms.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-4">
                            <div className="text-sm text-gray-900">{sms.recipient_phone}</div>
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                            {sms.message_type}
                          </td>
