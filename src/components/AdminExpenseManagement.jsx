...
const AdminExpenseManagement = () => {
  ...
  const handleApprove = async (expenseId) => {
    setProcessing(true);
    try {
      const response = await expenseAPI.updateExpenseStatus(expenseId, {
        status: "approved",
      });
      if (response.data.success) {
  ...
  const handleReject = async () => {
    ...
    setProcessing(true);
    try {
      const response = await expenseAPI.updateExpenseStatus(
        selectedExpense.id,
        {
          status: "rejected",
  ...
  const handleBulkApprove = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to approve ${selectedExpenses.length} expense(s)?`,
      )
    )
      return;
    ...
    try {
      ...
      showToast(
        error.response?.data?.message || "Failed to approve expenses",
        "error",
      );
    } finally {
      setProcessing(false);
  ...
  const handleBulkReject = async () => {
    ...
    const reason = window.prompt(
      "Enter rejection reason for all selected expenses:",
    );
    if (!reason) return;
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 text-xs font-semibold rounded-full border ${styles[status] || "bg-gray-100 text-gray-800"}`}
      >
  ...
  /**
   * Get count for a specific status from byStatus array
   * byStatus is now an array like: [{ status: 'pending', count: 5, total_amount: 1000 }, ...]
   */
  ...
  /**
   * Get total count across all statuses (for "All" tab)
   */
  const getAllTimeTotal = () => {
    if (!stats?.allTimeTotals) {
      // Fallback: sum up byStatus counts
      if (stats?.byStatus && Array.isArray(stats.byStatus)) {
        return stats.byStatus.reduce((sum, s) => sum + (s.count || 0), 0);
      }
      return 0;
    }
    return stats.allTimeTotals.totalCount || 0;
  };

  /**
   * Get approved amount for display (from byStatus or allTimeTotals)
   */
  const getApprovedAmount = () => {
    if (!stats?.byStatus || !Array.isArray(stats.byStatus)) return 0;
    const approvedData = stats.byStatus.find((s) => s.status === "approved");
    return approvedData?.total_amount || 0;
  };
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Review and approve agent expense submissions
            </p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {formatCurrency(getApprovedAmount())}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  {getTabCount("approved")} expenses
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  {stats?.totals?.totalCount || 0} entries
                </p>
              </div>
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${netProfit?.netProfit >= 0 ? "text-white" : "text-red-200"}`}
                >
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {selectedExpense.recorded_by_name || "N/A"}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {selectedExpense.property_name || "General"}
                    </p>
                  </div>
                  ...
                  {selectedExpense.approved_by_name && (
                    <div className="col-span-2 p-3 bg-gray-50 rounded-lg">
                      ...
                      <p className="font-medium text-gray-900">
                        {selectedExpense.approved_by_name} on{" "}
                        {formatDateTime(selectedExpense.approved_at)}
                      </p>
                    </div>
                  )}
              ...
              <div className="p-6">
                <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {selectedExpense.category} •{" "}
                    {selectedExpense.recorded_by_name}
                  </p>
                </div>
