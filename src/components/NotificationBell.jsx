...
import { API } from '../api'; // Your central API file
import { Bell, CheckCircle, XCircle } from 'lucide-react'; // Assuming you're using lucide-react for icons
...
const NotificationBell = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const unread = res.data?.filter((n) => !n.is_read).length || 0;
  ...
  const fetchChatUnread = async () => {
    try {
      const res = await API.chatAPI.getUnreadCount();
  ...
  const markAllChatsRead = async () => {
    try {
      ...
      console.error('Failed to mark all chats read:', error);
    }
  };
          ...
          <ul className="divide-y divide-gray-200">
            ...
            {notifications.map((n) => (
              <li
                key={n.id}
                className={clsx(
                  'p-4 hover:bg-gray-50 cursor-pointer flex items-start',
            ...
            {chatUnreadCount > 0 && (
              <li
                className="p-4 hover:bg-gray-50 cursor-pointer flex items-start bg-gray-50"
                onClick={markAllChatsRead}
              >
