...
class CronService {
  ...
  async start() {
    try {
          ...
          try {
            ...
            for (const row of rows) {
              ...
              try {
                ...
                // The reconciliation passes a mock res that only needs .status().json()
                // because the real handler ACKs immediately then processes asynchronously.
                const mockRes = {
                  status: function(code) {
                    return {
                      json: function(data) {
                        // ACK received — no-op, processing happens after
                      }
                    };
                  }
                };
      ...
      // ==================== RENT ARREARS SYNC — DISABLED ====================
      // This cron had a fundamentally broken formula that was corrupting arrears data:
      // - Assumed EVERY month from lease start was owed (ignoring actual payments per month)
      // - Ignored water bills in calculation
      // - Double-counted carry-forward payments
      // - Ignored advance payments
      // - Overwrote correctly-maintained arrears_balance with garbage
      // Arrears are already maintained incrementally by the payment allocation system
      // (trackRentPayment in paymentController.js). No recalculation needed.
      // If arrears drift, repair via payment allocation, not formula override.
      console.log("⚠️ Rent arrears sync cron DISABLED — arrears maintained by payment allocation system");
