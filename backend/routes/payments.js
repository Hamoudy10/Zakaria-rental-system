...
router.get('/mpesa/status/:checkoutRequestId', protect, paymentController.checkPaymentStatus);
...
router.get('/unit/:unitCode/status', protect, paymentController.getPaymentStatusByUnitCode);

// ==================== SALARY PAYMENT ROUTES ====================

// Process salary payment
router.post('/salary', protect, adminOnly, paymentController.processSalaryPayment);

// Get all salary payments
router.get('/salary', protect, paymentController.getSalaryPayments);

// Get salary payments for specific agent
router.get('/salary/agent/:agentId', protect, paymentController.getAgentSalaryPayments);

// ==================== REMINDERS ROUTES ====================

// Send balance reminders
router.post('/send-reminders', protect, adminOnly, paymentController.sendBalanceReminders);

// Get overdue reminders list
router.get('/reminders/overdue', protect, paymentController.getOverdueReminders);

// Send overdue reminders
router.post('/reminders/overdue', protect, adminOnly, paymentController.sendOverdueReminders);

// Get upcoming reminders list
router.get('/reminders/upcoming', protect, paymentController.getUpcomingReminders);

// Send upcoming reminders
router.post('/reminders/upcoming', protect, adminOnly, paymentController.sendUpcomingReminders);
...
router.get('/history/:tenantId', protect, paymentController.getTenantPaymentHistory);

// Get payments by tenant ID
router.get('/tenant/:tenantId', protect, paymentController.getPaymentsByTenant);

// Get payment summary for tenant + unit
router.get('/summary/:tenantId/:unitId', protect, paymentController.getPaymentSummary);

// Get detailed payment history for tenant + unit
router.get('/details/:tenantId/:unitId', protect, paymentController.getPaymentHistory);
...
router.put('/:id', protect, async (req, res) => {
  ...
  try {
    ...
    const paymentCheck = await client.query('SELECT id FROM rent_payments WHERE id = $1', [id]);
